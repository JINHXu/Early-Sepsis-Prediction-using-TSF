...
def get_t_sus(sus_win, IV, cultures) -> float:
    ...
    # try if this only makes possible combinations -> greatly reduce the amount of combinations
    ...
    for x in l: #for x in l:
        ...
        elif x[0] < x[1] and (x[1] - x[0]) <= sus_win[0]:
            t_.append(x[0])
        # cultures first and within sus_window[1] of IV
        elif x[1] < x[0] and (x[0] - x[1]) <= sus_win[1]:
            t_.append(x[1])
...
def is_septic(sofa, sus, sep_win) -> bool:
    ...
    for x in l:
        if (x[1] - x[0]) <= sep_win[0] or (x[0] - x[1]) <= sep_win[1]:
