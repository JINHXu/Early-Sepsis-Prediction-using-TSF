...
def get_trim_predict(trimmed_df, predicted_vals, map):
    ...
    for col in trimmed_df.columns:
        ...
        if col == "anti":
            if (
                predicted_vals[map.index("Antibiotics")] >= 2.001330
        ...
        if col == "mech":
            if (
                predicted_vals[map.index("Mechanically ventilated")] >= 2.0006310
        ...
        if col == "blood_culture":
            if (
                predicted_vals[map.index("Blood Culture")] >= 2.002810
...
def run_Sepsis(
    ...
    if run_on == "no-prediction":
        ...
        df = prepare_strats_for_sepsis(patient_data, features)
        df = fill_data(df, ffill)
        observed = df.loc[df.index < 120]
        print("standard mode")
        prepare_for_sofa(observed)
        get_sofa(observed)
        sepsis_label, t_sepsis, t_sofa, t_cultures, t_IV, t_sus = get_sepsis(
            observed, mode, sus_window, sep_window
        )
        s = dict(
            {
                "Subject ID": subject_ID,
