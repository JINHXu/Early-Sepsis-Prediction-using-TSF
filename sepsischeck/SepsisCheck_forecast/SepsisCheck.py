...
def sepsis_check(param: Sepsis, mode, sus_window, sep_window) -> int:
    ...
    t_sus = get_t_sus(sus_win=sus_window, IV=t_IV, cultures=t_cultures)  # returns hour

    if not t_sofa == False:
        t_sofa = t[t_sofa]  # get the hour corresponding to row number

    sepsis_label, t_sepsis = is_septic(t_sofa, t_sus, sep_win=sep_window)
...
def get_t_sus(sus_win, IV, cultures) -> float:
    ...
    elif IV < cultures and cultures - IV <= sus_win[0]:
        return IV
    # cultures first and within sus_window[1] of IV
    elif cultures < IV and IV - cultures <= sus_win[1]:
        return cultures
